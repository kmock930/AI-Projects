...
has_nose(X) :- % checks whether the individual has a nose.
    individual(X),
    isa(X,person).
height_of(X,163) :- % checks the height of an individual if he/she's solely a person.
    individual(X),
    isa(X,person),
    \+isa(X,man),
...
height_of(X,195) :- % checks the height of an individual if he/she's a baseball_player.
    individual(X),
    isa(X,baseball_player).
